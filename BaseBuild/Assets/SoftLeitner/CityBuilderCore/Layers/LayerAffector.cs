...
{
    /// <summary>
    /// affector that uses the points of an <see cref="IStructure"/> on the same or the parent component
    /// </summary>
    public class LayerAffector : MonoBehaviour, ILayerAffector
    {
        ...
        protected virtual void Start()
        {
            ...
            _structure.PositionsChanged += structurePositionsChanged;
        ...
        {
            Dictionary<Vector2Int, int> values = new Dictionary<Vector2Int, int>();
            ...
            foreach (var item in _structure.GetPoints())
            {
                ...
                {
                    foreach (var point in PositionHelper.GetRing(item, i))
                    {
