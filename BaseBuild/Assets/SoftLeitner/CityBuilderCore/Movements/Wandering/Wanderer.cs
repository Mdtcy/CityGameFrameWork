...
namespace CityBuilderCore
{
    public class Wanderer : MonoBehaviour, ISaveData
    {
        ...
        private void wander()
        {
            ...
            foreach (var position in PositionHelper.GetAreaAroundPositions(Dependencies.Get<IGridPositions>().GetGridPosition(transform.position), 1))
            {
                if (_tilemap == null || Tiles.Contains(_tilemap.GetTile((Vector3Int)position)))
                    candidates.Add(Dependencies.Get<IGridPositions>().GetWorldPosition(position));
            }
