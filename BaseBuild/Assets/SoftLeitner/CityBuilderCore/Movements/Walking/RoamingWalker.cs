    ...
    public class RoamingWalker : Walker
    {
        ...
        private void onRoamFinished()
        {
            if (ReturnHome)
            {
                ...
                tryWalk(() => Dependencies.Get<IRoadPathfinder>().FindPath(_current, _start, PathTag), planned: () => _state = RoamingWalkerState.Returning);
            }
            else
            {
                onFinished();
        ...
        {
            return getDescription((int)_state, _home.Instance.GetName());
        }
