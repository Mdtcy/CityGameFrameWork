...
namespace CityBuilderCore
{
    ...
    public class DefaultPopulationManager : MonoBehaviour, IPopulationManager, IEmploymentManager, IWorkplaceFinder
    {
        ...
        public WorkerPath GetWorkerPath(IBuilding building, Vector2Int? currentPoint, Worker worker, ItemStorage storage, float maxDistance, PathType pathType, object pathTag)
        {
            ...
            List<BuildingComponentReference<IWorkerUser>> candidates = new List<BuildingComponentReference<IWorkerUser>>();
            foreach (var reference in Dependencies.Get<IBuildingManager>().GetBuildingTraitReferences<IWorkerUser>())
            {
                var distance = Vector2.Distance(reference.Instance.Building.WorldCenter, building.WorldCenter);
                if (distance > maxDistance)
                    continue;

                if (reference.Instance.GetWorkerNeed(worker) == 0f)
                    continue;
            ...
            if (candidates.Count == 1)
            {
                ...
                var path = PathHelper.FindPath(building, currentPoint, user.Instance.Building, pathType, pathTag);
                if (path == null)
                    return null;

                WorkerPath workerPath = new WorkerPath(user, path);

                var items = candidates[0].Instance.GetItemsNeeded(worker);
                if (items != null)
                {
            ...
            else if (candidates.Count > 1)
            {
                ...
                foreach (var user in candidates)
                {
                    ...
                    var items = user.Instance.GetItemsNeeded(worker);
                    if (items != null)
                    {
                        var supplyPath = Dependencies.Get<IGiverPathfinder>().GetGiverPath(building, currentPoint, items, maxDistance, pathType, pathTag);
                        if (supplyPath == null)
                    ...
                    var score = user.Instance.GetWorkerNeed(worker);
                    if (score > maxScore)
                    {
                        maxScore = score;
